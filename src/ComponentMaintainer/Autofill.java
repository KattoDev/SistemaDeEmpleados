...
public class Autofill {
  ...
  public void LoadEmployeeList(Connection dbConnection, JTextField field) {
    try {
      PreparedStatement stmt = dbConnection.prepareStatement("SELECT usr_name FROM users");
      ResultSet rs = stmt.executeQuery();
      while (rs.next()) {
        employees.add(rs.getString("usr_name"));
      }
    ...
    AutoCompleteDecorator.decorate(
        employeeslist, field, ObjectToStringConverter.DEFAULT_IMPLEMENTATION);
  }
  ...
  public void LoadProjectList(Connection dbConnection, JTextField field) {
    ...
    AutoCompleteDecorator.decorate(
        projectsList, field, ObjectToStringConverter.DEFAULT_IMPLEMENTATION);
  }
  ...
  public void LoadDepartmentList(Connection dbConnection, JTextField field) {
    ...
    AutoCompleteDecorator.decorate(
        departmentList, field, ObjectToStringConverter.DEFAULT_IMPLEMENTATION);
  }
